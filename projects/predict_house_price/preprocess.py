...
ds = common_utils.cast_hf_dataset_columns(
    ds, datasets.Value("string"), categorical_columns
)
...
ds = ds.map(
    common_utils.one_hot_encode,
    fn_kwargs={"dict_maps": dict_maps, "column_names": categorical_columns},
    num_proc=4,
)
...
ds = ds.map(
    common_utils.replace_column_value,
    fn_kwargs={
...
# Save to csv
ds.save_to_disk("test_processed")

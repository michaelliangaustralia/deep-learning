...
class AudioTaggingModel(nn.Module):
    def __init__(self):
        ...
        self.sigmoid = (
            nn.Sigmoid()
        )  # sigmoid, not softmax as this is multi label classification

    def forward(self, audio):
        audio = audio.unsqueeze(
            1
        )  # N, C, H, W - https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html
        x = self.conv1(audio)
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.conv4(x)
        ...
        x = self.relu(
            self.linear1(x)
        )  # the relus make all the difference in helping the network learn non-linearities?
        x = self.relu(self.linear2(x))
        x = self.linear3(
            x
        )  # no relu on the last layer since we're doing classification
        x = self.sigmoid(x)
        return x

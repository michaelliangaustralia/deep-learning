...
def categorical_encode(
    row: Dict[str, Any], str_to_idx_dict: Dict[str, Dict[str, Any]]
) -> Dict[str, Any]:
...
def convert_to_int(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    row["RoomService_int"] = (
        int(float(row["RoomService"])) if row["RoomService"] != "nan" else 0
    )
    row["FoodCourt_int"] = (
        int(float(row["FoodCourt"])) if row["FoodCourt"] != "nan" else 0
    )
    row["ShoppingMall_int"] = (
        int(float(row["ShoppingMall"])) if row["ShoppingMall"] != "nan" else 0
    )
...
def fill_none(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    row["PassengerId_no_null"] = (
        str(row["PassengerId"]) if type(row["PassengerId"]) != type(None) else "nan"
    )
    row["HomePlanet_no_null"] = (
        str(row["HomePlanet"]) if type(row["HomePlanet"]) != type(None) else "nan"
    )
    row["CryoSleep_no_null"] = (
        str(row["CryoSleep"]) if type(row["CryoSleep"]) != type(None) else "nan"
    )
    row["Cabin_no_null"] = (
        str(row["Cabin"]) if type(row["Cabin"]) != type(None) else "nan"
    )
    row["Destination_no_null"] = (
        str(row["Destination"]) if type(row["Destination"]) != type(None) else "nan"
    )
    ...
    row["RoomService_no_null"] = (
        str(row["RoomService"]) if type(row["RoomService"]) != type(None) else "nan"
    )
    row["FoodCourt_no_null"] = (
        str(row["FoodCourt"]) if type(row["FoodCourt"]) != type(None) else "nan"
    )
    row["ShoppingMall_no_null"] = (
        str(row["ShoppingMall"]) if type(row["ShoppingMall"]) != type(None) else "nan"
    )
    row["Spa_no_null"] = str(row["Spa"]) if type(row["Spa"]) != type(None) else "nan"
    row["VRDeck_no_null"] = (
        str(row["VRDeck"]) if type(row["VRDeck"]) != type(None) else "nan"
    )
